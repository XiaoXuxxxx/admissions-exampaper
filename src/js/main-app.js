		...
		success: function(r){
			...
			for(var i=0;i<resp.length;i++){
				...
				tb_his.row.add([
					...
					"<button onclick='delExamData("+resp[i].id+",`"+resp[i].subject+"`,"+parseInt(i+1)+")' class='btn btn-sm btn-danger'><i class='fa fa-times'></i></button> ",
				]).draw(false)
			}
		}
	})
	   ...
	   dangerMode: true,
	    ...
	    if (resp) {
	      $.ajax({
	        ...
	        success:function(resp){
	          if(resp == "200"){
	            swal("ลบสำเร็จ",no+". "+exam_subject(subject)+" แล้ว",'success').then(()=>{
		            if($("#role").val() == 1){
		              getHistory()
		            }else{
		              getExamData()
...
function getExamData(){
	...
	$.ajax({
		url: "/api/getExamData.php?exam_type="+$('#exam_type').val() + "&subject=" + $('#exam_subject').val(),
		method : "GET",
		success: function(r){
			if(r == "404"){
				...
				for(var i=0;i<resp.length;i++){
					...
					tb_his.row.add([
						...
						resp[i].paper + "+" + resp[i].paper_extra + "="+ sumPaper,
						resp[i].username,
						"<button onclick='VerifyData("+resp[i].ide+")' "+disabled+" class='btn btn-sm "+btn_type+"'>Verify</button> "
						+"<button onclick='viewData("+resp[i].ide+")' class='btn btn-sm btn-secondary'><i class='fa fa-eye'></i> </button> " 
						+"<button onclick='delExamData("+resp[i].ide+",`"+resp[i].subject+"`,"+parseInt(i+1)+")' class='btn btn-sm btn-danger'><i class='fa fa-times'></i></button> ",
					]).draw(false)

					/* Count Section! */
					$.ajax({
						url : "/api/count.php?exam_type="+$('#exam_type').val() + "&subject=" + $('#exam_subject').val(),
						method: "GET",
						success: function(resp){
							var r = JSON.parse(resp)
							if($("#exam_type").val() == 0){
